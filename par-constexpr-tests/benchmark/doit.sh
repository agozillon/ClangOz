...
function runprog {
  ...
  local num_sizes=$6
  local i j k delta total avg serial
  ...
  # here is the output for the ideal speedup line (with (0,0) added at the start)
  echo 0 0
  for (( i = 0; i < num_core_sizes; i++ )); do
    echo ${cores[$i]} ${cores[$i]}
  done
  ...
  for (( i = 0; i < num_core_sizes; i++ )); do
    ...
    for (( j = 0; j < num_sizes; j++ )); do
      ...
      avg=$(echo "scale=5; $total / $nruns" | bc -l)  # 5 is the fp precision
      echo $avg ${sizes[$j]} | tee -a ${basename}.dat
      echo OK ${cores[$i]} $(echo "scale=5; ${serial[$j]} / $avg" | bc -l)

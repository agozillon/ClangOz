...
constexpr auto adjacency_ov1() {
  ...
  std::adjacent_difference(arr.begin(), arr.end(), out.begin());
  
  return out;
}

template <typename T, int N, int Offset, bool ForceRuntime = false>
constexpr auto adjacency_ov2() {
  if constexpr (ForceRuntime)
    std::cout << "is constant evaluated: " << std::is_constant_evaluated() << "\n";
  ...
  // the input and output probably can't alias
  std::adjacent_difference(arr.begin(), arr.end(), 
                           out.begin(), std::plus<>{});
...
int main() {
  constexpr auto output_ov1 = adjacency_ov1<int, 32, 2>();
  
  pce::utility::print_arr(output_ov1);
  
  constexpr auto output_ov2 = adjacency_ov2<int, 32, 2>();
  
  pce::utility::print_arr(output_ov2);

  auto runtime_ov1 = adjacency_ov1<int, 32, 2, true>();
  auto runtime_ov2 = adjacency_ov2<int, 32, 2, true>();

...
constexpr auto move_backward_ov1(auto& arr) {
  ...
  if constexpr (ForceRuntime) {
    ...
    std::move_backward(arr.begin(), arr.end(), arr_move.end());
  } else {
...
int main() {
  ...
  constexpr auto output_ov1 = move_backward_ov1<arr_t, arr_sz>(arr);
  auto runtime_ov1 = move_backward_ov1<arr_t, arr_sz, true>(arr2);

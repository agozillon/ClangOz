...
constexpr auto any_of_ov1() {
  ...
  if constexpr (ForceRuntime) {
    ...
    t1 = std::any_of(arr.begin(), arr.end(), 
                     [](auto i){ return (i % 2 == 0); });
    
    t2 = std::any_of(arr.begin(), arr.end(), 
                     [](auto i){ return (i % 2 == 1); });
  } else {
    // the input and output probably can't alias
    t1 = std::any_of(execution::ce_par, arr.begin(), arr.end(), 
                    [](auto i){ return (i % 2 == 0); });
    
    t2 = std::any_of(execution::ce_par, arr.begin(), arr.end(), 
                     [](auto i){ return (i % 2 == 1); });
  }

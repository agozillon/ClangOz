...
constexpr auto all_of_ov1() {
  ...
  if constexpr (ForceRuntime) { 
    ...
    t1 = std::all_of(vec.begin(), vec.end(), 
                          [](auto i){ return (i % 2 == 0); });
  
    t2 = std::all_of(vec.begin(), vec.end(), 
                          [](auto i){ return (i % 2 == 1); });
  } else {
    // the input and output probably can't alias
    t1 = std::all_of(execution::ce_par, vec.begin(), vec.end(), 
                          [](auto i){ return (i % 2 == 0); });
  
    t2 = std::all_of(execution::ce_par, vec.begin(), vec.end(), 
                          [](auto i){ return (i % 2 == 1); });
  }
...
int main() {
  constexpr auto output_ov1 = all_of_ov1<int, 32, 2>();
  auto runtime_ov1 = all_of_ov1<int, 32, 2, true>();
